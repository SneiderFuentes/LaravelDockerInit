...
class ProcessPatientRescheduleNotification implements ShouldQueue
{
    ...
    {
        Log::error('ProcessPatientRescheduleNotification job failed completely', [
    ...
    public function handle(GetUpcomingAppointmentsByPatientForSpecificAgendaHandler $handler): void
    {
        ...
        $this->processPatientRescheduleNotification($appointments);
    }

    private function processPatientRescheduleNotification(array $appointments): void
    {
        ...
        $consolidatedData = [
            ...
            'appointment_date_new' => $this->formatFriendlyDate($appointments[0]['date']),
            'appointment_time_new' => $appointments[0]['time_slot'],
            'appointment_date_cancel' => $this->formatFriendlyDate($this->previousDate),
        ...
        Log::info('Consolidated reschedule notification data', [
            'consolidated_data' => $consolidatedData
        ]);

        \Core\Jobs\SendRescheduleWhatsappMessage::dispatch($consolidatedData, $this->centerKey)
